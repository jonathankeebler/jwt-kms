            ...
            try {
                this.kms.describeKey({ KeyId: key_arn }, (err, data) => {
                    if (err) {
                        ...
                        if (headers.alg.toString() == "") {

                            let keys = Object.keys(this.awsmapping);

                            let found = false;
                            for (let i = 0; i < keys.length; i++) {
                                var key = keys[i];
                                var value = this.awsmapping[key];

                                if (data.KeyMetadata.SigningAlgorithms.includes(value)) {
                                    headers.alg = key;
                                    found = true;
                                    break;
                                }
                        ...
                        let params = {
                            ...
                            SigningAlgorithm: this.awsmapping[headers.alg],
                            MessageType: "RAW"
                        };
        ...
        return new Promise((resolve, reject) => {
            ...
            this.validate(token, function(err, components) {
                ...
                this.kms.verify(params, function(err, data) {
                    ...
                    let text = JSON.parse(base64url.decode(components.encrypted.payload));
                    resolve(text);
                });
            }.bind(this));
