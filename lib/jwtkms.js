...
class JWTKMS
{
    constructor(options) 
    {
		...
		this.hmac_secret = options.secret || "secret";
	}
	
	hmac()
	{
		return crypto.createHmac("sha256", this.hmac_secret);
	}
    ...
    sign(payload, options, key_arn)
    {
        ...
        return new Promise((resolve, reject) => {
			...
			let signature = token_components.header + "." + token_components.payload;
			signature = this.hmac().update(signature).digest("base64");

            this.kms.encrypt({
                Plaintext: new Buffer(signature, "base64"),
                KeyId: key_arn
            }, function(err, data)
            {
        ...
        return new Promise((resolve, reject) => {
			...
			this.validate(token, function(err, components)
			{
				...
				}, function(err, data)
				{
					...
					var payload_signature = components.encrypted.header + "." + components.encrypted.payload;

					// Old style signature
					if(base64url.decode(decrypted_signature) == payload_signature)
					{
						return resolve(components.payload);
					}
					// New style signature
					else if(decrypted_signature == this.hmac().update(payload_signature).digest("base64"))
					{
						return resolve(components.payload);
					}
					else
